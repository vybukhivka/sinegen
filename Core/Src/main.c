...
volatile uint32_t shared_encoder_value = 0;
...
int main(void) {
  ...
  encoderQueueHandle =
      osMessageQueueNew(16, sizeof(uint16_t), &encoderQueue_attributes);
...
void StartEncoderTask(void *argument) {
  ...
  /* Infinite loop */
  for (;;) {
    enc = __HAL_TIM_GET_COUNTER(&htim1);
    direction = __HAL_TIM_IS_TIM_COUNTING_DOWN(&htim1);
    osMessageQueuePut(encoderQueueHandle, &enc, 0, 0);
    osDelay(20);
  }
  /* USER CODE END 5 */
}
...
void StartDacTask(void *argument) {
  ...
  /* Infinite loop */
  for (;;) {
    if (osMessageQueueGet(encoderQueueHandle, &enc, NULL, osWaitForever) ==
        osOK) {
      if (osMutexAcquire(encoderMutexHandle, osWaitForever) == osOK) {
        shared_encoder_value = enc;
        osMutexRelease(encoderMutexHandle);
      }
      uint32_t dac_value = enc >> 4;
